...
| 11a | Benefits of Wrapper Classes? | **Null Values:** Unlike primitive types, wrapper classes can represent null values. This is useful in situations where the absence of a value needs to be explicitly indicated, such as when interacting with databases or APIs. **Utility Methods:** Wrapper classes provide various utility methods for tasks like converting between strings and primitive types (parseInt(), valueOf()), comparing values (compareTo()), and obtaining hash codes (hashCode()).| `String a = 10; int parsedNum = Integer.parseInt(a);` |
